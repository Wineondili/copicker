...
- Repaired the no-task official-control proxy for the current markerless power picker and exact non-Power Work picker: separated surface and actionable-item selectors, required exact trigger/`aria-controls` ownership, added semantic flyout/leaf discovery, strictly classified both three-flyout variants and the alternate flat-effort layout, excluded exactly one labeled Daybreak program prefix from those structural rows, used the exact current Model row as a catalog fallback when responsive layout hides the compact trigger label, used pointer-down plus a guarded legacy click fallback, and boundedly waits out the compact Ultra warning while Advanced is intentionally inert.
- Made model identity, transaction rollback, and Speed confirmation fail closed. The proxy preflights the target and current Model/Effort leaves, requires two identical current Model/Effort/tier snapshots, binds every mutation to the original still-unsent composer, and restores the captured current-build Speed leaf by index, count, and semantic signature—including `Ultrafast`—after a partial failure. Responsive icon-only triggers confirm through the exact checked Model leaf. A visually checked Standard can mask an unknowable unsupported carried tier, so CoPicker explicitly normalizes it through an observed catalog-resolved Fast-to-Standard transition before target mutation and repeats that normalization for unchanged Standard selections; a later failure restores that normalized Standard baseline and reports `restored-normalized`, not restoration of the hidden raw tier. The requested tier is conservatively established on the current model before Model/Effort changes and then re-applied on the target model. A current model without that Speed/Fast path, or a hidden current model that may disappear after switching, aborts before target mutation.
- Tightened identity, queue, and existing-task confirmation as separate gates: task-ID-only composer reuse invalidates prior confirmation; passive official-control refresh and Daybreak classification are serialized behind commits and retried for the same still-current composer; only explicit `serviceTier: null` means Standard; missing and unknown tier values invalidate matching waiters; notifications received before catalog loading are replayed; hidden adapted notifications remain recognizable; external notifications invalidate older queued intents; and generation is rechecked immediately before the request and after every awaited restore so a stale response cannot overwrite newer authoritative state. Existing-task confirmation now requires a strictly newer notification than the dispatched request; a post-dispatch timeout or otherwise unconfirmed outcome invalidates confirmation and displays `Other` instead of pretending the prior value was restored. Trusted official picker actions invalidate cached state across both notification-before-DOM and DOM-before-notification ordering.
- Recorded the current-build Daybreak boundary instead of silently proxying the wrong control: build `7377` removes `gpt-daybreak-blue-latest` from the Model submenu and exposes a separate generic Daybreak program checkbox that may remap both the current and default base model. Renderer `0.12.12` does not activate that broader control without an explicit base-model policy: it rejects the Daybreak row when the program control is present and also rejects ordinary model commits while that program is enabled or ambiguous. Ordinary model mutation remains available only when the exact program control is explicitly off. When neither that checkbox nor one exact legacy Daybreak Model leaf is observable, build `7377` provides no bounded signal that separates no entitlement from unresolved verified access, so every mutation-bearing selection fails closed. An exact legacy leaf is classified only for passive read-only display; mutation is deliberately refused on both task paths because its availability cannot be held atomically while Codex cycles mutually exclusive Model/Effort/Speed flyouts.
- Advanced the renderer development candidate to `0.12.12`, retained the previously user-confirmed click/drag easing and release-coordinate commit semantics, and synchronized the accepted baseline, architecture, usage, development, validation, settings, design-QA, and documentation-version contracts.
- Added executable layout classification, ambiguity, exact-label, service-tier, and Speed-confirmation coverage plus current selector/activation/catalog source contracts. Passed all 44 offline tests, JavaScript and shell syntax checks, whitespace validation, and the production release build. No installation, injection, Codex restart, live selection, task creation, LaunchAgent change, push, tag, or release was performed.
...
- Relocated the repository to `[local reference omitted]` while preserving its Git history.
