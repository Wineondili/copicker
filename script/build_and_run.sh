...
case "$MODE" in
  ...
  --autostart-disable|autostart-disable)
    "$APP_BINARY" autostart disable
    ;;
  --autostart-status|autostart-status)
    "$APP_BINARY" autostart status
    ;;
  --probe|probe)
    "$APP_BINARY" probe
    ;;
  ...
  --verify|verify)
    ...
    ;;
  *)
    echo "usage: $0 [run|--inject|--remove|--autostart-enable|--autostart-disable|--autostart-status|--probe|--probe-picker|--probe-primary|--probe-selector|--debug|--logs|--telemetry|--verify]" >&2
    exit 2
    ;;
esac
