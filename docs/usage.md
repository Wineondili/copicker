...
This guide describes the full-feature `v0.99.0` pre-release with CLI/plugin `0.99.0` and renderer `0.12.8`. Its unchanged renderer behavior was live-accepted at runtime commit `c127509` under the earlier CLI label `0.12.0-dev`, against Codex `26.820.60940` build `7119`. See [accepted-baseline.md](accepted-baseline.md) for exact requirement IDs and measurements.
