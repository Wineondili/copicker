...
This guide covers the stable `v1.0.0` release with renderer `0.12.20` and settings resource v4. It includes the build-`8881` picker, native settings initialization, Spark lifecycle notice, version-prefixed rail labels, and the corrected native settings background and scroll insets. Model routing was user-confirmed in the earlier picker pass; the user confirmed the actual settings interface on 2026-09-13 after the manual-restart handoff. The older `v0.99.0` renderer `0.12.8` and runtime anchor `c0343d4` remain historical. Installation, user acceptance, cold login, and publication remain separate gates. See [accepted-baseline.md](accepted-baseline.md) for exact requirements and historical results.
