...
| Renderer development candidate | `0.12.12` | `model-rail.js` `VERSION`; restored click/drag positional easing plus the build-`7377` no-task official-control repair remain offline/static only until a separately authorized live pass |
...
The public `v0.99.0` pre-release packages the full six-model implementation at renderer `0.12.8`. Renderer `0.12.9` was installed for a focused live pass: rapid pointer release passed, but the new-unsent-task official-control proxy did not. Renderer `0.12.10` changed only the official menu-item compatibility anchor from a button-specific selector to the then-observed tag-agnostic `data-list-navigation-item="true"` selector. Renderer `0.12.11` restored the original 240-millisecond click/drag positional easing without changing release-coordinate commits. Current `main` advances to `0.12.12`, retaining those interaction fixes and adapting the no-task path to the statically inspected Codex `26.825.51511` build `7377` semantic flyout/leaf structure. None of these candidate changes may be described as live-accepted until the focused live gates are rerun. `v0.11.0` remains an immutable historical three-model release.
...
- Do not proxy build `7377`'s generic Daybreak program checkbox as though it were the `daybreak-blue` Model leaf. Reject the Daybreak row on either task path when that control is present; allow ordinary model commits only while the exact program control is explicitly off, and reject them while it is enabled, busy, disabled, or otherwise ambiguous. If neither that control nor one exact legacy Daybreak Model leaf is observable, reject every mutation-bearing selection. An exact legacy leaf is passive read-only recognition only; do not mutate through that topology because availability cannot be held atomically while mutually exclusive flyouts cycle. Supporting the program requires an explicit base-model/effort policy because the official transition may remap current and default model selections.
