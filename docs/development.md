...
- With an active task ID, use `thread/settings/update` and require the matching `thread/settings/updated` confirmation.
- Before a task exists, proxy only the exact official Model, Effort, and Speed controls and confirm the resulting official trigger state.
...
- Preserve the 120-millisecond keyboard commit delay, 180-millisecond open/close motion, and 420-millisecond placement return delay unless a later accepted requirement supersedes them.
