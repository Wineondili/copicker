...
| Renderer development candidate | `0.12.12` | Current `main` payload; retains original click/drag positional easing and adapts the no-task official-control proxy to the statically inspected build `7377`, not yet installed or live-accepted |
...
| Statically inspected Codex desktop | `26.825.51511` build `7377` | Bundle-on-disk compatibility evidence only; no Inspector attachment, injection, settings mutation, or live acceptance |
...
Current `main` advances the renderer candidate to `0.12.12`. Read-only source inspection of Codex `26.825.51511` build `7377` showed that the mounted power picker no longer exposes the earlier Model-row or navigation-item attributes, its composer trigger opens on pointer-down, and its Effort submenu no longer exposes the selected-effort marker. The same bundle retains an exact non-Power Work branch with three top-level Model/Effort/Speed flyouts. The candidate therefore separates surface classification from semantic official items, binds flyouts through the active panel and each trigger's `aria-controls`, supports both exact three-flyout variants plus the exact catalog-sized alternate flat-effort layout, scopes leaf/Speed confirmation to the owned menu, and confirms a responsive label-less trigger through the exact checked Model leaf. Exactly one leading row is excluded from structural classification only when it is explicitly labeled as the Daybreak program; any other additional row fails closed because it cannot be distinguished safely from an unexpected effort or program control. This candidate has no installation, live new-task interaction, restart, or cold-login acceptance yet.
...
- **CP-MOD-007 — Current-build Daybreak boundary.** In Codex build `7377`, the Daybreak program path intentionally removes `gpt-daybreak-blue-latest` from the official Model submenu and exposes a separate `menuitemcheckbox` that may remap both the current and configured default base model. Renderer `0.12.12` does not activate that broader program control without an accepted base-model/effort policy. When the control is present, the Daybreak row is rejected before either task path mutates settings; ordinary model commits are allowed only while the exact control is explicitly off and are rejected while it is enabled, busy, disabled, or otherwise ambiguous. When neither the exact checkbox nor one exact legacy Daybreak Model leaf is observable, all mutation-bearing selection fails closed because build `7377` exposes no bounded signal that separates no entitlement from unresolved verified access. An exact legacy leaf may be recognized for passive display only; renderer `0.12.12` refuses mutation through that topology on both task paths because availability cannot be held atomically while Codex cycles mutually exclusive Model/Effort/Speed flyouts. `thread/settings/update` alone cannot express build `7377`'s program state.
