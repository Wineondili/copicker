...
| Renderer development candidate | `0.12.13` | Current `main` payload; corrects the live-observed Standard-tier and legacy-Daybreak blockers while retaining the build-`7377` no-task proxy and original click/drag easing; not yet installed |
...
Current `main` advances to renderer `0.12.13`. It recognizes both `null` and the observed official `"default"` value as Standard while preserving rejection of other unrelated tiers. It also restores mutation through an exact legacy Daybreak Model topology and holds only the Daybreak program-generation invariant while mutually exclusive Model/Effort/Speed flyouts cycle. The separate Daybreak program checkbox remains fail-closed unless explicitly off. The responsive flat-effort fallback, transaction baselines, pointer animation, and release-coordinate commit state machine remain unchanged. This candidate has not yet been installed or user-tested.
