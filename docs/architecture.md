...
### Builds 8378 and 8881 model-list adapter
...
Renderer `0.12.16` permits unknown unselected radios, as observed with GPT-5.2 on build `8881`, without treating them as Default or selectable adapted rows. The one checked concrete radio must still resolve to a unique known model and carry the explicit selected marker; unknown selected and ambiguous states fail closed. Luna remains limited to the five verified native effort steps even when the account catalog lists six.
...
If an option, order, or confirmation is absent or ambiguous, the no-task path fails closed. An idle no-task composer does not run the official-control proxy: it may display `Other` until the user explicitly chooses a rail cell. That explicit action runs one serialized, bounded transaction. Proxy-owned flyout mutations are excluded from external Daybreak invalidation, duplicate in-flight classification requests coalesce without scheduling another pass, and the confirmed result survives a same-composer trigger remount. After task creation, the renderer immediately returns to the direct task settings path.

Build `7377` removes `gpt-daybreak-blue-latest` from the official Model submenu only when its separate Daybreak program is available. That program is exposed as a generic checkbox and its official transition may remap both the current and configured default normal model before persisting the program flag. CoPicker does not treat the checkbox as a Model leaf or click it implicitly. When the program control is present, CoPicker rejects the Daybreak row; an ordinary model commit is allowed only while the exact control is explicitly off and is rejected while Daybreak is enabled, busy, disabled, or otherwise unreadable. When one exact legacy Daybreak Model leaf is present instead, CoPicker treats it as the normal model-backed topology, permits mutation, and requires the program control to remain absent across the transaction while ignoring expected structural churn from mutually exclusive flyouts. When neither topology is observable, mutation fails closed. Supporting the separate program requires a product decision for base model and effort.
...
The render tool advertises `ui://copicker/settings/v4.html` with the MCP App MIME type, themed data-URI icons, and empty network/frame allowlists. The previous v3 and v2 URIs remain read aliases for retained tool catalogs. All tools use `_meta.ui.visibility: ["app"]`; they are host controls, not general model-callable tools.

The settings document owns a full-height surface and an internal padded scroll viewport, rather than relying on body padding that the native sandbox resets. Native embedding starts at the right pane's top and needs its own 46 px toolbar spacer; the parent-controlled fallback already starts below that toolbar and suppresses the spacer through its existing controller marker. Both paths retain 20 px content padding and the 768 px column. The page background follows the native surface color independently of the user's rail appearance preference.
