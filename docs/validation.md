...
| Current `main` renderer | `0.12.18`; adds only a Spark Retiring notice while preserving native settings initialization and the user-accepted model switching from `0.12.16` |
...
| Current installed delivery | Release executable and Retiring-annotated HTML/renderer resources match source SHA-256; the user-authorized `0.12.18` watcher update reports successful injection without restarting Codex; the user's accepted preference snapshot and LaunchAgent plist are unchanged |
