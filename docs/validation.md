...
| Current `main` renderer | `0.12.20`; owns native settings background and vertical insets outside the sandbox body reset, preserving the rail and its labels |
...
| Current installed delivery | Release executable and HTML/renderer resources match source SHA-256; the user-authorized `0.12.20` watcher update reports successful injection on unchanged Codex PID (omitted). Preferences and the LaunchAgent plist remain byte-identical |
