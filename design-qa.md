...
- The native CoPicker portal instead starts at `y=0`. Its sandbox appends a second stylesheet with `html, body, #root { padding: 0; }`, overriding the document's body padding. Both html and body are transparent, exposing the wrong underlying surface. The old document's logical heading top and bottom content inset were zero. These are native-host regressions that previous mock initialization tests did not cover.
- Renderer `0.12.20` / resource v4 owns background and spacing on a full-height descendant shell, with a 46 px spacer and an internal 20 px padded scroll viewport. The parent-controller fallback marker disables that spacer because its host already begins below the native toolbar. The fallback copies the native surface token rather than preferring the unrelated tertiary surface. Model rail behavior, all labels, controls, and preference persistence are unchanged.
- Actual-source HTML in the in-app Browser, with the later sandbox reset reproduced: `1171 × 810` dark frame, shell background `rgb(24,24,24)`, heading `y=66`, scroll viewport `y=46` and height 764, 768 px content width, 20 px computed top/bottom padding. Scrolling to the end leaves 19.797 CSS px after subpixel rounding, with no document-level scroll. Mock appearance saving increments the in-memory revision. Fallback heading `y=20` plus host offset 46 gives the same global 66 px; light mode is white; `390 × 844` has no horizontal overflow. An initial Browser/sandbox MutationObserver exception did not recur on a scoped reload; the settings source contains no MutationObserver use.
- All 58 offline tests and the release build passed. Source/release/installed hashes, fresh v4/v3/v2 resource reads, unchanged preferences, and unchanged LaunchAgent configuration were verified. Installation and MCP refresh were explicitly authorized; Codex stayed on PID (omitted), with no forced process termination or restart. A refresh with unchanged server configuration left old services active. The registered plugin now includes a resource-URI environment marker tied to the Swift URI by a test; after configuration refresh, the actual native threadless resource read returned the v4 surface. The already-rendered native document remained a separate cache/acceptance check.
- Followed the existing installation workflow to re-register only CoPicker, then verified its actual installed plugin-cache manifest matches source (not just the local marketplace copy). Authorized configuration refresh and the native v4 read passed, but user-driven Browser → CoPicker navigation still retained the old document without `#copicker-settings-shell`, with logical heading top zero. Browser-page reload menu commands were disabled for this native entry. Stopped before any forced service termination, private UI-cache mutation, or Codex restart; the native visual gate remains pending a user-owned app restart/recreation.

result: root cause, source/build, isolated layout, installation, and actual native MCP resource delivery verified; native page presentation not yet passed because the existing window retains its old page instance
