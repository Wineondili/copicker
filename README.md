...
The selector and watcher behavior frozen in Copicker `0.11.0` is live-verified on Apple silicon against Codex `26.818.41705` (build `6971`), including guarded automatic injection after Codex restarts and Inspector shutdown after injection. The current development renderer payload uses the independent `0.12.1` compatibility identifier so it can replace older injected renderer state without renaming the legacy hook keys. Autostart remains opt-in and is never enabled by the default read-only command.
